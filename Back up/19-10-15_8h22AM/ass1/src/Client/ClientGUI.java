...
public class ClientGUI {
	...
	JButton Jconf;
	String userC;
	public JLabel lCall = new JLabel("dadad");
	...
	private void initialize() {
		...
		btnCall.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				if(btnCall.getText().equals("Call")){
					...
					portVoice = findFreePort();
					btnCall.setText("Waiting....");
					try {
						a.out.writeUTF("__AA__RECALL__AA__"+index+"__AA__"+IP+"__AA__"+portVoice+"__AA__"+userC);
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
				...
				if(btnCall.getText().equals("Answer")){
					accept = true;
				}
		...
		btnCall.setBounds(10, 138, 89, 23);
		friend.add(btnCall);
		btnCall.setUI(new BEButtonUI (). setNormalColor (BEButtonUI.NormalColor.green));
		
		JButton btnCancel = new JButton("Cancel");
		btnCancel.setBounds(104, 138, 89, 23);
		friend.add(btnCancel);
		
		
		lCall.setBounds(20, 111, 160, 16);
		friend.add(lCall);
