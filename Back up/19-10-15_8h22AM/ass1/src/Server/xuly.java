	...
	public synchronized String comand(String msg, ManaFile mf, Socket sk,Main m) {
		...
		if(p.length>1){
			...
			if(p[1].equals("RECALL")){
				...
				try {
					mnc.listClient.get(index).out.writeUTF("__AA__RECALL__AA__"+ip+"__AA__"+port+"__AA__"+p[5]);
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			...
			if(p[1].equals("DENY")){
				for(int i=0;i<mnc.listClient.size();i++){
					if(mnc.listClient.get(i).username.equals(p[2])){
						try {
							mnc.listClient.get(i).out.writeUTF("__AA__DENY");
						} catch (IOException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
			...
			if(p[1].equals("ACCEPT")){
				for(int i=0;i<mnc.listClient.size();i++){
					if(mnc.listClient.get(i).username.equals(p[2])){
						try {
							mnc.listClient.get(i).out.writeUTF("__AA__ACCEPT");
						} catch (IOException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
